...
def construir_documento(motor: TriageEngine) -> str:
    ...
    <section class="queixa">
      ...
      <div class="pergunta">
        <p class="meta">Mensagem do encaminhamento direto ao hospital, laranja e amarelo (modelo; v0.12.1)</p>
        <p>"Dirija-se a Hospital Dr. Nélio Mendonça ([X] km por estrada, ~[Y]
        min de carro). Nos casos classificados como [cor], o encaminhamento é
        feito diretamente para a urgência do hospital. Se os sintomas
        agravarem pelo caminho, ligue 112."</p>
      </div>
      <div class="caixa-validacao">
...
<body>
  <div class="capa">
    ...
    com o hospital como referência). A política é editável em
    app/data/encaminhamento.json, e um desfecho AMARELO pode declarar
    "destino": "atendimento_urgente" para voltar à urgência aberta mais
    próxima — esses desfechos aparecem marcados nas árvores abaixo com
    "(pode ir ao atendimento urgente)". No Porto Santo mantém-se a regra da
    ilha. Verde e azul não mudam.</p>
    <p><strong>Tempos de espera:</strong> nos desfechos que sigam pelo caminho
    por proximidade (por exemplo, amarelos com a exceção acima), o
    encaminhamento pode ainda considerar os tempos de espera em tempo real do
    SESARAM (regra experimental com salvaguardas: só troca a unidade mais
    próxima se poupar ≥30 min e o desvio for ≤15 km) — igualmente por
    validar.</p>
    <table class="cores">
